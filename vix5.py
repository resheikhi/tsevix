...
try:
    ...
    # ذخیره پاسخ دریافتی برای بررسی دستی
    with open("response_dump.xlsx", "wb") as f:
        f.write(response.content)
    print("📁 Response saved to response_dump.xlsx")

    # بررسی اینکه آیا واقعاً فایل اکسل است
    #if "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" in content_type:
    df = pd.read_csv(BytesIO(response.content))
    print("✅ Excel loaded successfully")
    print(df.head())
   # else:
    #    print("⚠️ Response is not an Excel file. Check response_dump.xlsx manually.")
...
def compute_combined_vix(df):
    ...
    for T in T_values:
        ...
        total_variance += variance * T  # Weighted by time to maturity
        weights += T
